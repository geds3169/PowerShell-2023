...
Description:
Ce script surveille les dossiers %AppData%\Local et %AppData%\Roaming\Microsoft\Windows\Start Menu\Programs\Startup pour détecter les exécutables associés au ransomware Big Head.
Une fois détectés, Il recherche les processus correspondant et les tues puis les exécutables sont supprimés du système.
...
Note: Ce script utilise les commandes suivante:
      - Get-process et Stop-Process pour lister les différents processus et tuer les exécutables recherchés.
      - Unblock-File et Remove-Item pour supprimer les exécutables détectés. Assurez-vous de comprendre les implications de ces commandes et de les utiliser avec précaution.
...
$action = {
    ...
    # Tue le processus lié à l'exécutable
    try { 
        $process = Get-process | select ProcessName, Id, CPU, Path | Where {$_.ProcessName -Match $Filter } | Stop-Process -Force
        Write-Host "Attention, un processus lié au ransomware (Big Head) a été détecté et tué : $process" -ForegroundColor Green
    } catch {
